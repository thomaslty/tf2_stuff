...
new String:DJ_FeetParticles[MAX_PLAYERS_ARRAY][MAX_EFFECT_NAME_LENGTH];	// Death (arg11)
new Float:DJ_ParticleDuration[MAX_PLAYERS_ARRAY];	// Death (arg12)
...
public Action:Event_RoundStart(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	for (new clientIdx = 1; clientIdx < MAX_PLAYERS; clientIdx++)
	{
		...
		if (FF2_HasAbility(bossIdx, this_plugin_name, DJ_STRING))
		{
			...
			FF2_GetAbilityArgumentString(bossIdx, this_plugin_name, DJ_STRING, 11, DJ_FeetParticles[clientIdx], sizeof(DJ_FeetParticles[])); // Death
			DJ_ParticleDuration[clientIdx] = FF2_GetAbilityArgumentFloat(bossIdx, this_plugin_name, DJ_STRING, 12); // Death
	...
	if (DJ_OnCooldownUntil[clientIdx] == FAR_FUTURE)
	{
		...
		if (DJ_CrouchOrAltFireDownSince[clientIdx] != FAR_FUTURE && (buttons & validButtons) == 0)
		{
			...
			if (eyeAngles[0] <= JUMP_TELEPORT_MAX_ANGLE && !IsInInvalidCondition(clientIdx))
			{
				...
				if(!IsEmptyString(DJ_FeetParticles[clientIdx]))
				{
					AttachParticleToFeet(clientIdx, DJ_FeetParticles[clientIdx], "foot_L");
					AttachParticleToFeet(clientIdx, DJ_FeetParticles[clientIdx], "foot_R");
				}
...
public DSSG_RemoveSentry(sentryIdx)
{
	if (DSSG_ParticleEntRef[sentryIdx] != INVALID_ENTREF)
		Timer_RemoveEntity(INVALID_HANDLE, DSSG_ParticleEntRef[sentryIdx]);
...
public DJ_ChangeFundamentalStats(clientIdx, Float:chargeTime, Float:cooldown, Float:multiplier, affectWeighdown)
{
	if (chargeTime != -1.0)
	...
	if(affectWeighdown != -1)
		DJ_DontAffectWeighdown[clientIdx] = bool:affectWeighdown;
}
...
stock ParticleEffectAt(Float:position[3], String:effectName[], Float:duration = 0.1)
{
	...
	if (particle != -1)
	{
		...
		if (duration)
		{
			...
			Format(output, sizeof(output), "OnUser1 !self:Kill::%f:1", duration);
...
stock AttachParticleToFeet(clientIdx, String:effectName[], String:attachPoint[])
{
	new particle = CreateEntityByName("info_particle_system");

	new String:target[128];
	Format(target, sizeof(target), "target%i", clientIdx);
	DispatchKeyValue(clientIdx, "targetname", target);
	...
	SetEntPropEnt(particle, Prop_Send, "m_hOwnerEntity", clientIdx);
	
	Format(target, sizeof(target), "OnUser1 !self:Kill::%f:1", DJ_ParticleDuration[clientIdx]);
	SetVariantString(target);
	AcceptEntityInput(particle, "AddOutput");
	AcceptEntityInput(particle, "FireUser1");
